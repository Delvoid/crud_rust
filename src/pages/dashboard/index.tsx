...
export default function Home() {
  ...
  return (
    <AuthedLayout>
      {vis ? (
        <EditModal vis={vis} setVis={setVis} id={id} setId={setId} />
      ) : null}
      <div className="bg-stone-200 shadow-md mt-10 w-[100%-2rem] md:w-4/5 min-h-[20rem] rounded-md bg-sky-300 px-10 flex flex-col items-center">
        ...
        {data.length > 0 ? (
          <table className="w-[100%] px-4 border-collapse">
            ...
            {data.map((record: record, index) => (
              <tr
                key={index}
                className="border-collapse grid grid-cols-11 grid-rows-auto"
              >
                ...
                <td className="col-span-1 flex flex-row justify-center">
                  {name == record.owner ? (
                    <button
                      data-id={record.id}
                      onClick={handleEdit}
                      className="w-full px-4 text-center bg-stone-200 hover:bg-stone-100 transition-all"
                    >
                      Edit
                    </button>
                  ) : null}
          ...
          <p> There are no notes here :( Maybe try creating one? </p>
        )}
      </div>
    </AuthedLayout>
  );
